...
import { RetrievalRequest, RetrievalResponse } from '../models';
...
export class QueryPageComponent {
  ...
  readonly form = signal<RetrievalRequest>({
    query: '',
    top_k: 5,
    rerank: true,
  ...
  readonly result = signal<RetrievalResponse | null>(null);

  updateField<K extends keyof RetrievalRequest>(key: K, value: RetrievalRequest[K]): void {
  ...
  run(): void {
    ...
    this.api.retrieve(request).subscribe({
      next: (result) => {
        ...
        this.history.addEntry(this.history.createEntry('query', request, result, 'success'));
      },
      error: (error) => {
        const message = this.toErrorMessage(error);
        this.error.set(message);
        this.history.addEntry(this.history.createEntry('query', request, null, 'error', message));
      },
      complete: () => {
  ...
  private sanitizeRequest(value: RetrievalRequest): RetrievalRequest {
    return {
      ...value,
